# VECAD FPGA Chess — DE2-115 / PS/2

**Current demo:** DE2-115 with a keyboard plugged into its PS/2 connector, an
external VGA monitor, a blue cursor (`#3366FF`), automatic board rotation, and on-screen restart confirmation.
The player to move is always at the bottom. Arrows follow screen directions;
pieces and labels stay upright. Promotion finishes before rotation; new games start with White at the bottom.
The laptop is needed only for programming; no laptop controller runs during play.

```powershell
python tools/build.py --board de2-115
python tools/program.py --board de2-115
```

Or program `output_files/de2_115/chess_de2_115.sof` using Quartus Programmer.
Double-click **Program DE2-115.cmd** for the checked programming script.

Use arrows/WASD, Enter/Space, Esc, and 1–4 for promotion. **F2 opens NEW GAME;
Enter confirms, Esc cancels.** KEY0 resets immediately. Connect the PS/2 keyboard
before powering the board. Use a native PS/2 keyboard or a keyboard specifically
supporting PS/2; a passive adapter does not convert an ordinary USB-only keyboard.

See [DE2-115 implementation, diagnostics and verification](docs/de2_115.md).
The original working DE10-Lite source/bitstream package is preserved in
`releases/de10_lite_verified_2026-09-09.zip`. Both targets share the chess engine,
but have separate board wrappers and output directories. The tools retain their
old default for compatibility: **always pass `--board de2-115` for the new board**.

## Historical DE10-Lite instructions

The following applies to the DE10-Lite target. Rebuild its current shared sources
before using the original programming command, or use the archived package.
...
See [architecture](docs/architecture.md) and [presentation walkthrough](docs/demo.md).
